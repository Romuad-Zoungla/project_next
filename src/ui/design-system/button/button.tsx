...
switch (variant) {
    case "accent": //DEFAULT
     ...
     break;
     case "outline": 
     variantStyles = "bg-white  hover:bg-gray-400/50 border border-gray  text-gray-900 rounded"
  break;
  case "disabled": 
...
switch (size) {
    ...
    break;
    case "medium": //DEFAULT
        sizeStyles = `text-caption2 font-medium ${   variant==="ico" ? "w-[50px] h-[50px]" : "px-[18px] py-[15px]"} `;
        icoSize=20;
    break;
    case "large":
        sizeStyles = `text-caption1 font-medium  ${   variant==="ico" ? "w-[60px] h-[60px]" : "px-[22px] py-[18px]"} `;
        icoSize=24;
    break;
    ...
    return ( 
        <button type="button"
        className={clsx(variantStyles, sizeStyles,  icoSize, isloading && "cursor-wait", "relative" )}  onClick={()=>console.log('click')}  disabled={disabled}
        > 
        {isloading && (
